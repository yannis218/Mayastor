  ...
  after((done) => {
    async.series(
      [
        ...
        (next) => {
          fs.unlink(aioFile, (err) => next()); // eslint-disable-line handle-callback-err
        },
        (next) => {
          if (doUring()) fs.unlink(uringFile, (err) => next()); // eslint-disable-line handle-callback-err
          else next();
        }
      ],
      (err) => {
